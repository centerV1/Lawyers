...
import { customersData,CustomerItem } from "@/data/Customers";
import { useTranslate } from "@/utils/useTranslate";
import { useTranslations } from "next-intl";

export default function OurCustomers() {
  const getLocalized = useTranslate<CustomerItem>();
            ...
            {customersData.map((item) => (
              ...
              >
                <div className="bg-white rounded-sm overflow-hidden shadow-lg border border-slate-100 flex flex-col h-full transition-all hover:shadow-xl p-4">
                  ...
                  <div className="pt-6 pb-4 text-center flex flex-col items-center grow">
                    <h4 className="text-[#1a3673] font-bold text-lg mb-2">
                      {getLocalized(item, "title")}
                    </h4>
                    <p className="text-slate-500 text-xs leading-relaxed max-w-55">
